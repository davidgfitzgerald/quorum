...
from quorum.core.connection import manager as connections
...
async def websocket_echo(websocket: WebSocket):
    client_id = await connections.connect(websocket)

    try:
        await websocket.send_json({"client_id": str(client_id)})
        while True:
            data = await websocket.receive_text()
            await connections.broadcast(f"Client #{client_id} says: {data}", client_id)
    except WebSocketDisconnect:
        await connections.disconnect(client_id)
        await connections.broadcast(f"Client #{client_id} left the chat", client_id)
...
async def websocket_rpc(websocket: WebSocket):
    ...
    try:
        ...
        await connections.disconnect(client_id, code=exit_code)
